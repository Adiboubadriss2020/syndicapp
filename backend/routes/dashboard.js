...
router.get('/stats', async (req, res) => {
  try {
    ...
    // Monthly revenues (from invoices with Payé status + client balances)
    ...
    const monthlyRevenuesFromInvoices = monthlyRevenuesResult ? Number(monthlyRevenuesResult) : 0;
    console.log('Monthly revenues from invoices:', monthlyRevenuesFromInvoices);
    
    // Also include client balances for current month (as they represent potential revenue)
    const clientBalancesResult = await Client.sum('balance', {
      where: { payment_status: 'Payé' }
    });
    const clientBalances = clientBalancesResult ? Number(clientBalancesResult) : 0;
    console.log('Client balances (Payé status):', clientBalances);
    
    // Total monthly revenues = invoices + client balances
    const monthlyRevenues = monthlyRevenuesFromInvoices + clientBalances;
    console.log('Total monthly revenues (invoices + balances):', monthlyRevenues);
    ...
    for (const monthKey of sortedMonths) {
      ...
      const revenuesFromInvoices = revenuesResult ? Number(revenuesResult) : 0;
      console.log(`Revenues from invoices for ${monthKey}:`, revenuesFromInvoices);
      
      // For current month, also include client balances
      let clientBalancesForMonth = 0;
      if (month === now.getMonth() + 1 && year === now.getFullYear()) {
        const clientBalancesResult = await Client.sum('balance', {
          where: { payment_status: 'Payé' }
        });
        clientBalancesForMonth = clientBalancesResult ? Number(clientBalancesResult) : 0;
        console.log(`Client balances for current month ${monthKey}:`, clientBalancesForMonth);
      }
      
      const totalRevenues = revenuesFromInvoices + clientBalancesForMonth;
      console.log(`Total revenues for ${monthKey}:`, totalRevenues);
      ...
      chartData.push({ 
        ...
        charges,
        net: totalRevenues - charges
      });
      
      console.log(`Chart data for ${monthKey}:`, { month: monthKey, revenues: totalRevenues, charges, net: totalRevenues - charges });
    }
